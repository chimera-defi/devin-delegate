#!/usr/bin/env python3
"""Audit devin-delegate links and doc adoption across repos."""
...
def audit(root: Path, skill_source: Path, *, include_self: bool = False, include_worktrees: bool = True) -> dict:
    ...
    for repo in iter_workspace_repos(root, include_worktrees=include_worktrees):
        if not include_self and repo.resolve() == skill_source.resolve():
            continue
        skill_link = repo / "skills" / "devin-delegate"
        ...
        rows.append(
            {
                ...
                "docs_with_devin_delegate": doc_hits,
